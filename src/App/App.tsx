...
const App: FC = () => {
  ...
  const PATH_URL = `https://api.themoviedb.org/3/${search.length === 0 ? "movie" : "search"}/${
    search.length === 0 ? "popular" : "movie"
  }?api_key=${API_KEY}&language=en-US&page=${page}&query=${search}\``;
  ...
  useEffect(() => {
    ...
    void fetchMovies(PATH_URL).then((res: FetchResults) => {
